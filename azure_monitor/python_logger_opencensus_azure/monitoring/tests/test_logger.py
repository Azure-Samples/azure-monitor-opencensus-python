...
class TestAppLogger(unittest.TestCase):
    ...
    def get_app_logger(self, config=None):

        app_logger = AppLogger(config or self.valid_config)
        return app_logger

    def test_app_logger_creation_invalid_instrumentation_key(self):
        """Test with invalid instrumentation key."""
        global test_invalid_instrumentation_key
        with self.assertRaises(Exception):
            logging.disable(logging.CRITICAL)
            app_logger = self.get_app_logger(self.invalid_config)
            app_logger.get_logger()
            assert app_logger is not None


    def test_app_logger_creation_no_instrumentation_key(self):
    ...
    def test_app_logger_creation_valid_instrumentation_key(self):
    ...
    def test_logging_level(self):
        ...
        try:
            ...
            app_logger = self.get_app_logger()
            app_logger.config = valid_config
